...
import { TeacherScoreTable } from "./table";
...
export default function TeacherScore() {
  ...
  createEffect(() => {
    if (exams()?.length) {
      setSelectedExamId(exams()!.at(1)!.id);
    }
  });

  const [fullScores, { mutate }] = createResource(selectedExamId, (examId) => {
    if (!examId) {
      return [];
    }
  ...
  return (
    <div class="space-y-8 px-12 pt-8">
      ...
      <Switch>
        ...
        <Match when={fullScores() && fullScores()!.length !== 0}>
          <TeacherScoreTable scores={fullScores()!} mutate={mutate} />
        </Match>
      </Switch>
    </div>
  );
